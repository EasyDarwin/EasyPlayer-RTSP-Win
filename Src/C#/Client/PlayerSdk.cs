...
public class PlayerSdk
{
    ...
    /// </summary>
    /// <returns>System.Int32.</returns>
    [DllImport(@"Lib\libEasyPlayer-RTSP.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "?EasyPlayer_Init@@YAHXZ")]
    ...
    {
        tagRECT rect = new tagRECT { left = lpSrcRect.left, bottom = lpSrcRect.bottom, right = lpSrcRect.bottom, top = lpSrcRect.top };
    ...
    /// <returns></returns>
    public static int EasyPlayer_PicShot(int channelId, string shotPath, string fileName)
    {
        int ret = -99;

        if (!System.IO.Directory.Exists(shotPath))//如果不存在就创建file文件夹　　             　　                
            System.IO.Directory.CreateDirectory(shotPath);//创建该文件夹

        ret = EasyPlayer_SetManuPicShotPath(channelId, shotPath, fileName);
        ret = EasyPlayer_StartManuPicShot(channelId);
        System.Threading.Thread.Sleep(200);//取50的时候太短，有时候会没有截图
    ...
    /// <returns>System.Int32.</returns>
    [DllImport(@"Lib\libEasyPlayer-RTSP.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "?EasyPlayer_SetManuPicShotPath@@YAHHPBD0@Z")]
    private static extern int EasyPlayer_SetManuPicShotPath(int channelId, string shotPath, string fileName);
    ...
    /// <returns></returns>
    public static int EasyPlayer_StartManuRecording(int channelId, string recordPath)
    {
        int ret = -99;

        if (!System.IO.Directory.Exists(recordPath))//如果不存在就创建file文件夹　　             　　                
            System.IO.Directory.CreateDirectory(recordPath);//创建该文件夹
        ret = EasyPlayer_SetManuRecordPath(channelId, recordPath);
    ...
    [DllImport(@"Lib\libEasyPlayer-RTSP.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "?EasyPlayer_ShowOSD@@YAHHHUtagEASY_PALYER_OSD@@@Z")]
    public static extern int EasyPlayer_ShowOSD(int channelId, int show, tagEASY_PALYER_OSD osd);
}
